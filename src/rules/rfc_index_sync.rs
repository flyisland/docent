...
use crate::model::{Project, RFC_DIR, Violation, id_from_rel, load_docs, relative_str};
use crate::rules::Rule;
use std::collections::{HashMap, HashSet};

pub struct RfcIndexSyncRule;

impl Rule for RfcIndexSyncRule {
    ...
    fn run(&self, project: &Project) -> Vec<Violation> {
        let mut v = Vec::new();
        let docs = load_docs::<crate::model::rfc::RfcFront>(&project.root, RFC_DIR, &["README.md"]);
        let mut by_id: HashMap<String, &crate::model::Loaded<crate::model::rfc::RfcFront>> =
            HashMap::new();
        for d in &docs {
            let id = d
                .front
        ...
        let index_path = project.rfc_index();
        ...
        for row in &table.rows {
            ...
            let Some(doc) = by_id.get(id) else {
                v.push(Violation::error(
                    ...
                    index_rel.clone(),
                    None,
                    format!("index lists {} but no matching RFC file exists", id),
                ));
                continue;
            };
            ...
            let linked = front.related_adr.clone().unwrap_or_else(|| "—".to_string());
            let title_col = table.col("Title");
            let status_col = table.col("Status");
            let date_col = table.col("Date");
            let linked_col = table.col("Linked ADR");
            ...
            if let Some(c) = date_col {
                want.push(("date", row.get(c).cloned(), Some(front.index_date())));
            }
            if let Some(c) = linked_col {
                want.push(("linked ADR", row.get(c).cloned(), Some(linked)));
            }
